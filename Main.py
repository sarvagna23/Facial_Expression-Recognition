...
def loadLibraries(): #function to load yolov3 model weight and class labels
    ...
    unconnected_layers = cnn_model.getUnconnectedOutLayers()  # Get unconnected layers
    cnn_layer_names = cnn_model.getUnconnectedOutLayersNames()  # Get unconnected layer names

    # Ensure the indices are valid for cnn_layer_names
    valid_indices = [idx - 1 for idx in unconnected_layers if 0 <= (idx - 1) < len(cnn_layer_names)]

    # Use valid indices to get the corresponding layer names
    cnn_layer_names = [cnn_layer_names[idx] for idx in valid_indices]
